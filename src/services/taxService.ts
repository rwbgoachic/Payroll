...
export class TaxService {
  ...
  private static taxRatesCircuitBreaker = new CircuitBreaker(5, 60000, 30000);

  /**
   * Calculate payroll taxes for a given gross pay amount and state
   */
  static async calculatePayrollTaxes(grossPay: number, state: string): Promise<TaxRates> {
    try {
      // Use circuit breaker to protect against API failures
      return await this.taxRatesCircuitBreaker.execute(async () => {
        ...
        const federalTax = this.calculateFederalTax(grossPay, federalRates?.[0]);

        // Calculate state tax
        const stateTax = this.calculateStateTax(grossPay, stateRates?.[0]);
   ...
   */
  static async getTaxRates(date: Date = new Date()): Promise<any[]> {
    return retry(async () => {
   ...
   */
  static async calculateTaxes(amount: number, state: string): Promise<TaxRates> {
    try {
      // Use circuit breaker to protect against API failures
      return await this.taxRatesCircuitBreaker.execute(async () => {
   ...
   */
  static getTaxCircuitBreakerState(): 'closed' | 'open' | 'half-open' {
    return this.taxRatesCircuitBreaker.getState();
  }

  /**
   * Reset the tax rates circuit breaker
   */
  static resetTaxCircuitBreaker(): void {
    this.taxRatesCircuitBreaker.reset();
  }

  /**
   * Calculate tax based on tax brackets
   */
  private static calculateFederalTax(grossPay: number, rateInfo: any): number {
    if (!rateInfo) {
   ...
   */
  private static calculateStateTax(grossPay: number, rateInfo: any): number {
    if (!rateInfo) {
   ...
   */
  static calculateTax(amount: number, taxRate: any): number {
   ...
   */
  private static calculateTaxWithBrackets(amount: number, brackets: any[]): number {
